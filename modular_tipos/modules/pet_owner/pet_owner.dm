...
/datum/quirk/item_quirk/pet_owner/add_unique(client/client_source)
	...
	if(desired_pet != "Random")
		pet_type = GLOB.possible_player_pet_tipos[desired_pet]

	if(pet_type == NONE) // Pet not set, we're picking one for them.
		pet_type = pick(flatten_list(GLOB.possible_player_pet_tipos))
...
GLOBAL_LIST_INIT(possible_player_pet_tipos, list(
	...
	"Capybara" = /mob/living/basic/pet/capybara,
...
/datum/preference/choiced/pet_owner/init_possible_values()
	return list("Random") + assoc_to_keys(GLOB.possible_player_pet_tipos)
